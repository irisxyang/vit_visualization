"""
Saliency-driven image compositor.

This is the runtime blending implementation. Called once per dwell
from the WS route (via app/pipeline/blending.py), and once during the
optional `_blend_overlays_only` path used by ad-hoc visualization
scripts.

Inputs:
  original  — PIL Image, numpy array, or path. Subject image.
  saliency  — np.ndarray [H, W] in [0, 1]. High = keep original.
  overlays  — 1-3 rep-max images (any of the above formats).
  modes     — blend mode strings, one per overlay.

Output:
  np.ndarray float32 [H, W, 4] — RGB + alpha. The alpha channel
  carries the saliency mask; the route ignores it for display.

Blend modes: normal, screen, multiply, overlay, hard_light,
soft_light, luminosity, difference.
"""
...
def composite(
    ...
    *,
    ...
    # ---- stage 1: blend overlays against each other (no original involved) ----
    # this is the fully-opaque "non-saliency" backdrop that fills regions
    # where the original is transparent.
    overlay_blend = _blend_overlays_only(base, overlays, modes, H, W)

    # ---- stage 2: alpha-composite the original over the overlay blend ----
    # mask doubles as alpha: where mask=1 we see only the original, where
    # mask=0 we see only the overlay blend (fully opaque, no base leakage).
    rgb = base * mask + overlay_blend * (1.0 - mask)

    # Alpha channel still carries the mask for callers who want to render
    # the original on a transparency background; the route ignores it.
