...
JSON all raise at startup, BEFORE the server starts handling requests.
This is by design — silent degradation hides bugs.
"""
...
class PatchData:
    ...
    saliency_display_url: str
    saliency_raw_url: str
...
def _load_one(image_id: str) -> ImageData:
    ...
    # also check the source image actually exists on disk
    ...
    # --- patches: must have all 196 entries ---
...
def _parse_original(d: dict, *, where: str) -> OriginalData:
    _require(d, ("class_id", "class_name", "top_3_channel_ids",
                 "saliency_display_url", "saliency_raw_url"), where)
    ...
    return OriginalData(
        ...
        saliency_display_url=str(d["saliency_display_url"]),
        saliency_raw_url=str(d["saliency_raw_url"]),
    )
